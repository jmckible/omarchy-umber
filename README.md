...
- `extension/content.js` — on every page and frame, applies the palette to `documentElement` and attaches matching site styles via `adoptedStyleSheets`, which sorts after the page's own sheets in the cascade. Everything reacts to `storage.onChanged`, so pages restyle live and the popup's per-site power button strips palette and styles the same way.
