...
- Host↔extension protocol (native messaging, all JSON): host pushes `{type:"palette"|"styles"|"generate-status"}`; extension sends `{type:"append", site, matches, css}` (picker save) and `{type:"agent", url, site, census, screenshot}` (launches the default agent in a terminal via `omarchy-agent-prompt`, pointed at that stylesheet). A headless `{type:"generate", url, site, census}` path still exists in the host but nothing in the UI sends it; its agent comes from the `omarchy-default-agent` slug mapped to a headless spelling, or the shell command in `~/.config/umber/agent` — contract: prompt on stdin, CSS on stdout. In every inbound message `site` names the **stylesheet**, not the page host: `extension/match.js` (`Umber.resolveStyle`) resolves it from the `@match` headers and falls back to the hostname only when nothing claims the URL — that is what makes the host's `-f $SITES_DIR/$site.css` test a truthful create-vs-refine check, and it keeps picker saves out of forked hostname files. The refine prompt deliberately differs from create: read the existing file, ask the user what to change, edit narrowly. Storage keys: `omarchyColors`, `omarchyStyles`, `omarchyEnabled`, `omarchyConnected`, `omarchyGenerate`.
